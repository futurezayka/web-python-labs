...
class GenreRepository(RepositoryMixin):
    ...
    async def get_multi(
        ...
        async with MongoUnitOfWork() as mongo_uow:
            items, count = await mongo_uow.genres.get_multi(offset=offset, limit=limit, order_by=order_by, **filters)
    ...
    async def create(self, obj_in: dict[str, any]):
        async with MongoUnitOfWork() as mongo_uow:
            await mongo_uow.genres.create(obj_in)
        await super().create(obj_in)

    async def delete(self, filters: dict[str, any]):
        async with MongoUnitOfWork() as mongo_uow:
            await mongo_uow.genres.delete(filters)
